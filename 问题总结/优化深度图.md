...
第二种是CopyDepth，渲染完非透明物体之后，将[depthbuffer](https://zhida.zhihu.com/search?q=depthbuffer&zhida_source=entity&is_preview=1)拷贝到深度纹理中

## 问题

URP在OPENGLES3.1模式中获取深度图的方式为，每个模型新增一个[depthpass](https://zhida.zhihu.com/search?q=depthpass&zhida_source=entity&is_preview=1)，在渲染非透明物体之前，渲染一遍depthpass，这个方式会导致[顶点](https://zhida.zhihu.com/search?q=顶点&zhida_source=entity&is_preview=1)计算翻倍，查阅了一些资料，Unity给出的解释是这个bug是OPENGL导致的，这个问题该由OPENGL来修复，到目前位置，Unity2023还是没有解决该问题。。。
